...
func validoAuthorization(ctx context.Context, request events.APIGatewayProxyRequest) (bool, int, string, models.Claim) {
	...
	if path == "registro" || path == "login" || path == "obtenerAvatar" || path == "obtenerBanner" {
		return true, 200, "", models.Claim{} //{} vacías indica que estamos devolviendo una estructura vacía
	}

	token := request.Headers["Authorization"] //endpoint
	if len(token) == 0 {
	...
	if !todoOK { //puede que el token esté expirado o no es correcto
		if err != nil {
